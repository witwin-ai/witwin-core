...
class Cone(GeometryBase):
    ...
    def signed_distance(self, xx, yy, zz):
        ...
        return convex_polygon_signed_distance_2d(profile_points, triangle).reshape(axial.shape)
...
class Prism(GeometryBase):
    ...
    def signed_distance(self, xx, yy, zz):
        ...
        polygon_distance = convex_polygon_signed_distance_2d(torch.stack([px, py], dim=-1).reshape(-1, 2), polygon).reshape(px.shape)
